...
def grade_task3(action: Action, ground_truth: dict) -> RewardSignal:
    ...
    if ideal_keywords:
        ...
        keyword_score = round((len(matched) / len(ideal_keywords)) * 0.45, 3)
        score += keyword_score
    ...
    score += min(len(tone_hits) * 0.1, 0.25)
    notes.append(f"tone signals: {tone_hits}")
    score += 0.15 if 20 <= word_count <= 120 else 0.05
    notes.append(f"word count: {word_count}")
    return RewardSignal(score=round(min(max(score, 0.05), 0.95), 3), done=True, info={"notes": notes, "word_count": word_count})
