...
def grade_task1(action: Action, ground_truth: dict) -> RewardSignal:
    ...
    if predicted is None or predicted.strip() == "":
        return RewardSignal(score=0.0, done=False, info={"reason": "empty label", "expected": expected, "got": predicted})
    ...
    if predicted == expected:
        return RewardSignal(score=1.0, done=True, info={"reason": "correct", "expected": expected, "got": predicted})
    return RewardSignal(score=0.0, done=True, info={"reason": "wrong label", "expected": expected, "got": predicted})
